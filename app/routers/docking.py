...
async def _run_docking_job(job_id: str, req: DockingRequest):
    ...
    try:
        ...
        if receptor_path is None:
            msg = (f"Receptor preparation failed for {protein_id}. "
                   "Refusing to return scores that did not come from Vina.")
            await log(job_id, f"[FAILED] {msg}", "warn")
            await update(job_id, "failed", 100, msg)
            return
        else:
...
def _simulate_poses(smiles_list: list, names: list) -> list:
    """Removed. Scores derived from SMILES string length were being returned
    to users flagged as real docking results."""
    raise RuntimeError("simulated scoring has been removed from NexusMD")
