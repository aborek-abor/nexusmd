...
Endpoints:
  ...
  POST /api/md/submit                               — submit an MD job (docking-based or complex)
  POST /api/md/submit-direct                        — submit an MD job (uploaded complex shorthand)
  GET  /api/md/status/{job_id}                      — poll job status
  GET  /api/md/poll/{job_id}                        — lightweight polling with logs
  GET  /api/md/results/{job_id}                     — fetch completed results
  GET  /api/md/results/{job_id}/trajectory          — download DCD file (frontend alias)
  GET  /api/md/results/{job_id}/download/trajectory — download DCD file (canonical)
"""
...
# ── Submit (docking-based or uploaded complex) ─────────────────────────────
...
# ── Lightweight poll (with logs) ───────────────────────────────────────────

@router.get("/poll/{job_id}")
async def poll_md_job(job_id: str):
    """
...
# ── Trajectory download (frontend alias: /results/{job_id}/trajectory) ─────

@router.get("/results/{job_id}/trajectory")
async def download_trajectory_alias(job_id: str):
    """Download the DCD trajectory file (frontend-facing alias)."""
    return await _serve_trajectory(job_id)


# ── Trajectory download (canonical: /results/{job_id}/download/trajectory) ─
...
async def _serve_trajectory(job_id: str):
    """Shared implementation for both trajectory download routes."""
